...
old_func_pattern = r"      function renderASTNode\(node, depth = 0\) \{.*?fileTreePane\.innerHTML = html;\s*\}"
...
if not old_func:
    ...
    new_func = """      const astStyles = `
      ...
      function renderASTNode(node, depth = 0) {
        ...
        let html = '';
        const lineLoc = (node.variable && node.variable.reference) ? ` <span style="color: var(--text-muted); font-size: 10px; margin-left: 6px; opacity: 0.7; font-weight: normal;">(Line ${node.variable.reference.line})</span>` : '';
        ...
        if (node.children && node.children.length > 0) {
          ...
          html += `
            <div class="${isRoot ? '' : 'ast-item-wrapper'}">
              <div class="ast-node" style="margin-top: ${isRoot ? '0' : '2px'}; padding: 4px 6px;">
      ...
      function renderDependencyTree(mod) {
        ...
        let html = astStyles;
        html += `<div style="font-size: 13px; font-weight: 700; color: #fff; padding: 8px; margin-bottom: 12px; border-bottom: 1px solid var(--border); word-break: break-all;">🌳 Intra-file AST Tree<br><span style="font-size: 11px; color: var(--text-muted); font-family: 'Fira Code', monospace; font-weight: normal;">${mod.id}</span></div>`;
